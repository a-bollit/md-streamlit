...
@Component({
  selector: 'app-forms-template-validation-on-type',
  templateUrl: './forms-template-validation-on-type.html',
  styleUrls: ['./forms-template-validation-on-type.scss'],
})
export class FormsTemplateValidationOnType implements OnInit {
  @ViewChild('clientForm', {static: false}) clientForm: any;
  ...
  addMessage(name: string, message: string) {
    ...
    if (this.clientForm.control.get(name).status !== 'VALID' ) {
