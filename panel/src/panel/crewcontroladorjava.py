...
import warnings
from crew import print_output_bucle
...
class javaCrew():
	...
	@agent
	def DTO_MAPPER_generator_agent(self) -> Agent:
		return Agent(
			config=self.agents_config['DTO_MAPPER_generator_agent'],
	...
	@agent
	def service_controller_generator_agent(self) -> Agent:
		return Agent(
			config=self.agents_config['service_controller_generator_agent'],
			allow_delegation=False,
			verbose=True,
			llm=self.llm,
		)
...
#-----------------------------------------------------------------------------------
	@task
	def DTO_MAPPER_generator_task(self) -> Task:
		return Task(
			config=self.tasks_config['DTO_MAPPER_generator_task'],
			agent=self.DTO_MAPPER_generator_agent(),
	...
	@task
	def service_controller_generator_task(self) -> Task:
		return Task(
			config=self.tasks_config['service_controller_generator_task'],
			agent=self.service_controller_generator_agent(),
			context=[self.DTO_MAPPER_generator_task()],
			callback=print_output_bucle,
			human_input=False,
		)
