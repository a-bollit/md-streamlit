...
class ConversationalRetrievalTool(BaseTool):
    ...
    def __init__(self, **data):  # Usa **data para recibir parámetros
        ...
        self.llm_interpreter = AzureChatOpenAI(api_key=api_key, deployment_name=deployment_model,api_version=api_version,azure_endpoint=azure_endpoint,temperature=0.0)
        self.embeddings = AzureOpenAIEmbeddings(azure_deployment=embeddings_model,azure_endpoint=embeddings_endpoint)

        project_root = os.path.abspath(os.path.join(os.path.dirname(__file__), '../../..'))
        path_files = os.path.join(project_root, 'src/panel/documentacion/mdx')
