...
@Component({
  selector: 'app-forms-reactive-validation-no-menu',
  templateUrl: './forms-reactive-validation-no-menu.html',
  styleUrls: ['./forms-reactive-validation-no-menu.scss'],
})
export class FormsReactiveValidationNoMenu implements OnInit {
  ...
  ngOnInit() {
    ...
    const myVarVal = docStyle.getPropertyValue('--border-color-components');
    console.log(myVarVal);
    this.changeMenu(false);

    // document.documentElement.style.setProperty(`--${border-color-components}`, value + suffix); //suffix may be px or ''
  }
  toggleMenuHeader = true;
  backWithMenu() {
    this.changeMenu(this.toggleMenuHeader);
    this.toggleMenuHeader = !this.toggleMenuHeader;
  }
  changeMenu(show) {
    const headerConfig = {
      showHeader: show,
      showLateralMenu: show,
    };
